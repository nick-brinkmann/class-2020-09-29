...
```{r scene_2}
read_enrollment <- function(file, start){
  read_excel(file, 
             skip = start) %>% 
  clean_names() %>% 
  filter(!is.na(course_title)) %>% 
...
d_2019_take_2 <- read_enrollment(file = "raw_data/class_enrollment_summary_by_term_12.20.19.xlsx", start = 3)
```
...
```{r scene_3}
d_2019 <- read_enrollment(file = "raw_data/class_enrollment_summary_by_term_12.20.19.xlsx", start = 3)
d_2018 <- read_enrollment(file = "raw_data/class_enrollment_summary_by_term_10.24.18.xlsx", start = 2)
d_2017 <- read_enrollment(file = "raw_data/class_enrollment_summary_by_term_10.20.17.xlsx", start = 3)
d_2016 <- read_enrollment(file = "raw_data/class_enrollment_summary_by_term_fall_2016.xlsx", start = 0)
d_2015 <- read_enrollment(file = "raw_data/class_enrollment_summary_by_term_fall_2015.xlsx", start = 3)
...
# ID's originally 1, 2, 3, 4, 5. We want 2015, 2016, 2017, 2018, 2019. So add 2014 to each year. ID column was originally a character vector. Use as.integer()
...
```{r scene_5}
course_list_2019 <- all %>%
  filter(year == 2019 & u_grad > 250)

course_names <- course_list_2019$title

course_names

all %>%
  filter(title %in% course_names) %>%
  ggplot(aes(x = year, y = u_grad)) +
  geom_col() +
  facet_wrap(~ title) +
  labs(title = "Changes in Enrollment in Large Harvard Courses In Last 5 Years", y = "Enrollment")
```
